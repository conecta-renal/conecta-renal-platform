...
arquivo de Estabelecimentos (ST) — a partir do FTP público do DATASUS,
para o Conecta Renal.

Uso:
    python ingest_cnes.py
...
(`cnes/ano={ano}/mes={mes}/data.parquet`) — não em disco local.

Nota sobre o arquivo:
    O CNES tem 13 sub-arquivos (Estabelecimentos, Dados Complementares,
    Profissionais, Leitos, Equipamentos, etc.). Este pipeline traz apenas
    o arquivo de **Estabelecimentos (ST)** — o cadastro principal, usado
    para enriquecer os códigos CNES já referenciados no SIH e no ATD
    (ex: nome/tipo/localização do estabelecimento onde o paciente foi
    atendido). Layout confirmado contra um arquivo real do FTP
    (`STSP2605.dbc`, 113.631 estabelecimentos, 208 colunas — próximo do
    Informe Técnico oficial do CNES, que documenta 203).

    Diferente do SIH/SIA-SUS, os arquivos de CNES ficam em uma subpasta
    própria por tipo (`Dados/ST`, `Dados/PF`, `Dados/LT`, etc.), não numa
    única pasta com tudo junto.
"""
...
# Subpasta especifica do arquivo de Estabelecimentos (ST) dentro do CNES.
...
def configurar_logger() -> logging.Logger:
    ...
    log_path = LOGS_DIR / f"cnes_{timestamp}.log"

    logger = logging.getLogger("ingest_cnes")
...
def imprimir_resumo(resumo: ResumoExecucao) -> None:
    ...
    print("\n===== Resumo da execução - CNES (Estabelecimentos) =====")
...
def main() -> None:
    ...
    logger.info("Iniciando ingestão CNES (ST) | UF=%s | janela=%s meses", uf, qtd_meses)
